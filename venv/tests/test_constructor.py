...
class TestMainFunctionality:
    ...
    def test_ingredient_counter_increase(self, browser_setup):
        ...
        print(f"Начальное значение каунтера галактического соуса: {initial_count}")
        ...
        print(f"Обновленное значение каунтера галактического соуса после добавления: {updated_count}")
    ...
    def test_logged_in_user_can_place_order(self, browser_setup, create_user):
        ...
        account_page.login_via_web(create_user["email"], create_user["password"])
        page.drag_and_drop_ingredient_to_constructor()
        page.place_order()
